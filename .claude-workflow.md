...
- **Host filter**: Filter hosts in SSH Hosts panel by name, hostname, or username
  - Supports plain text (substring) and glob patterns (`*`, `?`)
  - Filter/clear buttons in panel title bar (toggle based on `sshLite.hasHostFilter` context)
  - Entirely local — no server calls (LITE compliant)
...
- Auto-upload on save with **upload state tracking**:
  - Tab badge `↑` (yellow) while uploading to server via FileDecorationProvider
  - Tab badge `✗` (red) on upload failure — save again to retry
  - Status bar shows `$(sync~spin) Uploading filename...` during upload
  - Status bar shows `$(cloud-upload) Saved filename to server` on success
  - Status bar shows `$(error) Save failed: filename` on failure
  - Activity panel shows upload progress via CommandGuard
  - 60-second timeout on SFTP write for slow servers
...
- Smart conflict handling:
  ...
  - **Skip refresh while document is dirty** (user is editing) — no server fetch
  - **Skip refresh while upload is pending/in-flight** — no overwrites during save
  - Resumes automatically when document becomes clean (upload success or Ctrl+Z)
  - Prompt if remote changed while user has local changes (focused file only)
...
- Reveal in Tree button (📍) to navigate to file in explorer
...
### 10. Filename Filter
- Filter files in tree by glob pattern — non-matching files are **hidden** (not just highlighted)
...
- Filtered folder highlighted via `FileDecorationProvider` (blue icon+text, 'F' badge) with `[filter: pattern]` description
- **Empty folder graying**: Folders with no matching descendants are grayed out (icon + label text via `disabledForeground`)
  - `isEmptyAfterFilter()` checks if folder is under basePath and NOT in `highlightedPaths`
  - FileTreeItem sets gray icon; FileDecorationProvider grays label text for `ssh://` URIs
  - Auto-resets when filter is cleared
- **Reveal-in-tree auto-clear**: If revealed file would be hidden by filter, filter is auto-cleared
  - `revealFile()` checks `matchesFilenameFilter()` before navigating
  - `onFilterClearedCallback` syncs decoration provider and VS Code context
...
- Cross-session persistence via `.ssh-lite-index` file mapping backup → original paths
...
│   ├── connection/
│   │   ├── ConnectionManager.ts  # Manage multiple connections, auto-reconnect
│   │   └── SSHConnection.ts      # SSH/SFTP operations, file watching
│   ├── services/
│   │   ├── HostService.ts        # Parse SSH config, manage hosts
│   │   ├── FileService.ts        # File operations, auto-sync, backups
│   │   ├── TerminalService.ts    # SSH terminals
│   │   ├── PortForwardService.ts # Port forwarding
│   │   ├── CredentialService.ts  # Credential storage with pinned folders
│   │   ├── AuditService.ts       # Audit trail logging
│   │   ├── ServerMonitorService.ts # Server diagnostics, live monitor panel
│   │   ├── PriorityQueueService.ts # Priority-based task queue for preloading
│   │   ├── FolderHistoryService.ts # Track folder/file access history
│   │   ├── ProgressiveDownloadManager.ts # Large file progressive downloads
│   │   ├── ActivityService.ts    # Track running operations for Activity panel
│   │   └── CommandGuard.ts       # Man-in-the-middle guard for command/data tracking
│   ├── providers/
│   │   ├── HostTreeProvider.ts   # SSH hosts tree view with credentials
│   │   ├── FileTreeProvider.ts   # Remote files tree view with filter/highlight
│   │   ├── FileDecorationProvider.ts # Upload badges, connection state, filter highlight
│   │   ├── PortForwardTreeProvider.ts # Port forwards tree
│   │   ├── ActivityTreeProvider.ts # Activity panel tree view
│   │   └── ProgressiveFileContentProvider.ts # Virtual documents for previews
│   ├── webviews/
│   │   └── SearchPanel.ts        # VS Code-style search webview panel
│   ├── types/
...
// refreshSingleFile skips if:
// - document.isDirty (user is editing)
// - pendingUploads has the file (upload debounced)
// - pendingUploadPromises has the file (upload in-flight)
// - uploadingFiles has the file (tracked as uploading)
// Resumes automatically on next cycle when file is clean

// Save flow:
// 1. User Ctrl+S → VS Code local save → onDidSaveTextDocument
// 2. handleFileSave: capture content, mark uploading, show spinner, set ↑ badge
// 3. Debounced uploadFileWithAudit runs (60s write timeout)
// 4. Success → clear uploading state, remove badge, show status bar success
// 5. Failure → mark failed, show ✗ badge + error notification
```
...
### Automated Tests
- **Test framework**: Jest with ts-jest (`npm test`)
- **Total tests**: ~330 tests across 15 test suites
- **Run tests**: `npx jest --no-coverage` or `npm test`
- **Run with coverage**: `npx jest --coverage`
- **Run specific file**: `npx jest --testPathPattern=ConnectionManager`

### Test Infrastructure
- `src/__mocks__/vscode.ts` — VS Code API mock (EventEmitter, TreeItem, MarkdownString, window, workspace, commands)
- `src/__mocks__/testHelpers.ts` — Shared mock factories: `createMockConnection()`, `createMockHostConfig()`, `createMockRemoteFile()`, `createMockCredential()`, `createMockPinnedFolder()`, `createMockPortForward()`

### Unit Tests
- `src/services/ActivityService.test.ts` — Activity lifecycle, progress, grouping, cancel-all (25 tests)
- `src/services/CommandGuard.test.ts` — Exec/read/write/list tracking, failure tracking (18 tests)
- `src/services/FileService.test.ts` — Save flow simulator: skipNextSave, debounce, flush, close-reopen (20 tests)
- `src/services/CredentialService.test.ts` — Credential CRUD, session credentials (existing)
- `src/providers/FileTreeProvider.test.ts` — Filter matching, glob patterns (existing)
- `src/services/CredentialService.pinned.test.ts` — Pinned folder CRUD: add, delete, rename, get (~15 tests)
- `src/services/ServerMonitorService.test.ts` — Parse functions: server status, memory, disk, processes, diagnose (~35 tests)

### Integration & Service Tests
- `src/connection/ConnectionManager.test.ts` — Connect lifecycle, disconnect, auto-reconnect, getters (~30 tests)
- `src/providers/HostTreeProvider.test.ts` — Server grouping, tree items, icons, credentials, pinned folders, host filter (~44 tests)
- `src/providers/PortForwardTreeProvider.test.ts` — Add/remove forwards, filtering, cleanup (~12 tests)
- `src/services/PortForwardService.test.ts` — Forward/stop/batch operations, prompt flow (~20 tests)
- `src/webviews/SearchPanel.test.ts` — Scope management, search execution, cancellation, activity tracking (~25 tests)
- `src/services/FileService.crud.test.ts` — Delete flow, create folder, open file simulators (~20 tests)
- `src/integration/credential-connection.test.ts` — Cross-service: credential storage, tree display, pinned folder lifecycle (~15 tests)

### Test Patterns
- **Singleton reset**: `(ServiceClass as any)._instance = undefined` between tests
- **Mock factory**: `jest.mock('./Module', () => ({ ... }))` with `require()` inside factory for runtime mocks
- **Fake timers**: `jest.useFakeTimers()` for timer-dependent code (auto-reconnect, debounce)
- **Import ordering**: Mock variable declarations MUST come before `jest.mock()` calls; module-under-test imports MUST come after `jest.mock()` calls to avoid temporal dead zone errors
...
1. ~~**Write integration & e2e tests**~~: ✅ Done — ~330 tests across 15 suites covering all major features
...
### 2026-01-28
- **Host filter for SSH Hosts panel**: Filter hosts by name, hostname, or username
  - Added `setFilter()`, `getFilter()`, `clearFilter()`, `matchesFilter()` to `HostTreeProvider`
  - Matches against: display name, host:port server key, hostname, username (case-insensitive)
  - Supports plain text (substring match) and glob patterns (`*`, `?`)
  - Filter/clear buttons in panel title bar (toggle via `sshLite.hasHostFilter` context)
  - Registered `sshLite.filterHosts` and `sshLite.clearHostFilter` commands in extension.ts
  - LITE compliant: entirely local filtering, no server calls
  - Files: `src/providers/HostTreeProvider.ts`, `src/extension.ts`, `package.json`
  - **Tests**: 12 new tests for host filtering in `HostTreeProvider.test.ts`
- **Empty folder graying after filename filter**: Folders with no matching descendants are grayed out
  - Added `isEmptyAfterFilter()` method to FileTreeProvider — checks if folder is under basePath and NOT in `highlightedPaths`
  - Added `isEmptyAfterFilter` parameter to `FileTreeItem` constructor — sets `disabledForeground` icon for empty folders
  - Added `setFilenameFilterPaths()` to `FileDecorationProvider` — grays label text for empty folder `ssh://` URIs
  - Extension.ts passes highlighted paths to decoration provider after filter search completes
  - `clearFilteredFolder()` now also clears empty folder graying state
  - Files: `src/providers/FileTreeProvider.ts`, `src/providers/FileDecorationProvider.ts`, `src/extension.ts`
- **Reveal-in-tree auto-clears filename filter when file would be hidden**
  - `revealFile()` checks if the target file would be hidden by `matchesFilenameFilter()`
  - If hidden, calls `clearFilenameFilter()` which invokes `onFilterClearedCallback`
  - Callback (wired in extension.ts) clears decoration provider, VS Code context, and shows status bar message
  - Files: `src/providers/FileTreeProvider.ts`, `src/extension.ts`
- **Tests**: 16 new tests for empty folder graying
  - `FileTreeProvider.test.ts`: 8 tests for `isEmptyAfterFilter` logic (no filter, different connection, outside basePath, basePath itself, empty folders, highlighted folders, nested depths, multiple empties)
  - `FileDecorationProvider.test.ts`: 8 tests for empty folder decoration (no filter, gray out, not gray highlighted, outside basePath, different connection, priority over blue badge, clear, nested depths)
  - Files: `src/providers/FileTreeProvider.test.ts`, `src/providers/FileDecorationProvider.test.ts`
- **Filename filter hides non-matching files**: Filter now actually hides files instead of just highlighting
  - Added `matchesFilenameFilter()` method in FileTreeProvider to filter non-matching files in `getChildren()`
  - Directories always pass through (to allow navigation)
  - Applied in 3 getChildren locations: sub-directories, buildDirectoryItems, buildReconnectingDirectoryItems
  - Files: `src/providers/FileTreeProvider.ts`
- **Filtered folder uses FileDecorationProvider**: Proper indent + icon+text highlighting
  - Removed `iconPath` override from filtered folders — was breaking indent alignment
  - Added `setFilteredFolder()` / `clearFilteredFolder()` methods to `SSHFileDecorationProvider`
  - `ssh://` scheme URIs get blue color + 'F' badge decoration via FileDecorationProvider
  - Both icon and text are colored (vs old approach which only colored the icon)
  - Files: `src/providers/FileDecorationProvider.ts`, `src/providers/FileTreeProvider.ts`, `src/extension.ts`
- **3-state expand/collapse rotation**: VS Code-like cycling button in tree views + search panel
  - Tree views: Expand All (`$(expand-all)`) → Expand to First Level (`$(fold-down)`) → Collapse All (`$(collapse-all)`)
  - Search panel: `⊞` Expand All Details → `≡` Collapse to File Level → `⊟` Collapse All
  - Context variables: `sshLite.{view}.expandState` = 0, 1, 2
  - `expandFirstLevel` uses `list.collapseAll` + `TreeView.reveal(item, {expand:1})` for each top-level item
  - Search panel `applySearchExpandState()` manages `expandedFiles`/`expandedTreeNodes` Sets per view mode
  - Files: `package.json`, `src/extension.ts`, `src/webviews/SearchPanel.ts`
- **Search exclude pattern fix**: `*uat*` and glob patterns now correctly exclude both files and directories
  - Always apply both `--exclude` and `--exclude-dir` for every pattern (was heuristic-based)
  - Files: `src/connection/SSHConnection.ts`
- **Reveal-in-tree for loading files**: Status bar "Reveal in Tree" works during file download
  - `activeDownloads` changed from `Set<string>` to `Map<string, {connectionId, remotePath}>`
  - Added `getActiveDownloadInfo()` method and progressive preview URI parsing in reveal handler
  - Files: `src/services/FileService.ts`, `src/extension.ts`

### 2026-01-26
- **Upload state tracking via FileDecorationProvider** (replaced dangerous workspace-edit dirty hack):
  - Removed `markDocumentDirtyForUpload()` and `clearDocumentDirtyAfterUpload()` — they caused race conditions (async workspace edits during save handler conflicted with auto-save, corrupted content with trailing space)
  - New approach: `uploadingFiles` / `failedUploadFiles` Sets in FileService + tab badge via FileDecorationProvider
  - Tab badge `↑` (yellow) while uploading, `✗` (red) on failure
  - Status bar shows `$(sync~spin) Uploading filename...` during upload
  - Status bar shows `$(cloud-upload) Saved filename to server` on success
  - Status bar shows `$(error) Save failed: filename` on failure
  - Files: `src/services/FileService.ts`, `src/providers/FileDecorationProvider.ts`
- **SFTP write timeout**: 60-second timeout on `SSHConnection.writeFile()` for slow servers
  - Previously no timeout — write could hang forever on slow/stale connections
  - Files: `src/connection/SSHConnection.ts`
- **Live-refresh pauses during editing**: No server fetch while user is editing
  - `refreshSingleFile()` returns early when `document.isDirty` or upload pending
  - Applies to: global refresh timer, focused file poll timer, native watch events
  - Resumes automatically when document becomes clean (upload success or Ctrl+Z)
  - Files: `src/services/FileService.ts`
- **Tests: comprehensive integration & e2e tests**: ~200 new tests (330+ total) covering all major features
  - Shared test infrastructure: `testHelpers.ts` (mock factories), updated `vscode.ts` mock (MarkdownString, tabGroups)
  - ConnectionManager: connect lifecycle, disconnect, auto-reconnect, credential handling, activity tracking
  - HostTreeProvider: server grouping by host:port, tree item icons/states, credentials, pinned folders
  - PortForwardService + TreeProvider: forward/stop/batch operations, prompt flow, tree storage, cleanup
  - ServerMonitorService: parse functions (status, memory, disk, processes), diagnose thresholds
  - SearchPanel: scope management, parallel execution, cancellation with AbortController, activity tracking
  - FileService CRUD: delete flow, create folder, open file simulators
  - CredentialService pinned folders: add, delete, rename, get operations
  - Integration: credential-connection flow, tree display by connection state, pinned folder lifecycle
  - Files: 9 new test files + 2 infrastructure files
...
- **Tests: ActivityService, CommandGuard, FileService save flow**: 63 new tests (132 total)
  - `ActivityService.test.ts`: Lifecycle (start, complete, fail, cancel), progress, grouping, cancel-all
  - `CommandGuard.test.ts`: Exec/read/write/list tracking, failure tracking, command description extraction
  - `FileService.test.ts`: Save flow simulator — skipNextSave regression, debounce, flush, close-reopen flow
  - Files: `src/services/ActivityService.test.ts` (NEW), `src/services/CommandGuard.test.ts` (NEW), `src/services/FileService.test.ts` (NEW)
- **Search: multi-server grouping fix**: Server grouping rows now always appear when searching across multiple servers
  - `multiServer` flag now based on `scopeServers` (search scopes) instead of result servers — ensures grouping even when one server returns zero results
...
- **Fix: skipNextSave leak silently eating user saves**: Removed 5 `skipNextSave.add()` calls before `workbench.action.files.revert` commands
  ...
  - **Correct usages retained**: Lines 2582, 2740 — these use `document.save()` which DOES fire `onDidSaveTextDocument`
  - Files: `src/services/FileService.ts`
- **Fix: Saved files reverting on reopen**: Fixed race condition where saved remote files reverted to old content when closed and reopened
  - **Root cause 1**: `document.getText()` was called inside 500ms debounce setTimeout callback — if user closed tab before timer fired, the TextDocument was disposed and content was stale/empty
  - **Root cause 2**: Race condition on close+reopen — reopening triggered re-download from server before the debounced upload completed
...
- **Search improvements**:
  ...
  - **Tree view**: Toggle between list view (☰) and tree view (🌲) in results header
...
- **Search reveal in tree**: Navigate from search results to file in explorer
  - Added 📍 reveal button to file headers in both list and tree view
...
- **Fixed auto-reconnect on manual disconnect**: Critical race condition bug
  ...
  - Close event handler saw `undefined` for `isManualDisconnect` → triggered reconnect
...
- **File search scope support**: Search icon on files adds file to SEARCH IN section
  ...
  - Search scope list shows 📄 icon for files, 📁 icon for folders
  - grep can search within single file paths directly
  - Files: `src/webviews/SearchPanel.ts`, `src/extension.ts`
- **3-state expand/collapse rotation**: VS Code-like cycling button in all tree views + search panel
  - Tree views: Expand All → Expand to First Level → Collapse All (3-state cycle)
  - Search panel: Expand All Details → Collapse to File Level → Collapse All (3-state cycle)
  - Context variables: `sshLite.{view}.expandState` = 0 (collapsed), 1 (fully expanded), 2 (first level)
  - Tree views use `list.expandRecursively`, `list.collapseAll`, and `TreeView.reveal(item, {expand:1})` for first level
  - Search panel uses `expandedFiles`/`expandedTreeNodes` Sets to apply each state
  - Search panel button (`⊞`/`≡`/`⊟`) in results header, resets to state 2 on new search
  - Commands: `sshLite.expandAll`, `sshLite.expandFirstLevel`, `sshLite.collapseAll`
  - Icons: `$(expand-all)`, `$(fold-down)`, `$(collapse-all)`
  - Works on: SSH Hosts, Remote Files, Activity, Port Forwards, Search Panel
  - Files: `package.json`, `src/extension.ts`, `src/webviews/SearchPanel.ts`
...
- **Filename filter fix**: Filter now provides clear visual feedback
  - Bug: Filtered folder (basePath) was not added to `highlightedPaths` — no visual change on filter

...
**Version**: 0.3.0 | **Tests**: 823 across 30 suites
...
| 9 | **Search Panel** | Cross-server search, find files mode, server checkboxes, auto-connect, redundancy detection, include/exclude, list/tree view |
...
### SearchPanel (Cross-Server Search)
```typescript
searchPanel.setServerList(entries)                        // populate all servers
searchPanel.setAutoConnectCallback(callback)              // auto-connect disconnected servers
searchPanel.setAutoDisconnectCallback(callback)           // cleanup after search
...
searchPanel.updateServerConnection(connectionId, connected) // real-time connection updates
```
...
```bash
npx jest --no-coverage                          # Run all unit tests
npx jest -- HostTreeProvider                    # Run specific file
npx jest --testPathPattern=e2e --no-coverage    # Run Docker e2e tests (requires Docker)
```
...
- `SearchPanel.test.ts` — Scope management, cancellation, server list, redundancy, auto-connect/disconnect
- `cross-server-search.e2e.ts` — Docker-based e2e: real SSH connect, search, cancel, permissions
...
- **Fix "Invalid username" connection failure** (#1): Validate host config fields before connecting, clear error messages for corrupted settings
- **Fix reconnect loop on invalid config**: "Invalid username" now correctly detected as non-recoverable error, stops auto-reconnect
- **Fix race condition in Add User**: `saveHost()` now properly awaited before credential save
- **Host config validation**: Invalid saved hosts (missing name/host/username) filtered out on load with warnings
- **Improved connection logging**: Full host config logged on every connection attempt for easier issue diagnosis
- **Fix search multi-folder**: `setServerList` now preserves existing searchPaths and checked state
- **Fix sort by checked**: Sort by checked state alone (not requiring paths), toggle persists to globalState

## Previous Changes (v0.2.6)

- **Cross-server search**: Server checkboxes to select which servers to search, multi-path per server
- **Find Files mode**: Toggle between content search (grep) and filename search (find)
- **Auto-connect**: Disconnected servers with saved credentials auto-connect during search
- **Auto-disconnect**: Servers auto-connected with no results get disconnected after search
- **Redundancy detection**: Same-user child paths grayed out (skipped), cross-user overlaps warned
- **Server sort**: Toggle between checked-first and alphabetical, persisted to globalState
- **Remote process cleanup**: SIGTERM sent to remote processes on cancel before stream.close()
- **Docker e2e tests**: Real SSH environment testing with linuxserver/openssh-server

## Previous Changes (v0.2.5)

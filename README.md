...
**1.0.2** - Smoother SSH key and passphrase login, and a simpler way to add hosts. When a host has a **private key**, SSH Lite no longer pops a password box on the first connect, and it asks for a **key passphrase only when the key is actually encrypted** (an unencrypted key connects with no prompt at all); a wrong passphrase now shows a clear "incorrect passphrase" message with a one-click retry. **Adding a host is simpler**: you enter just the name, hostname, and port - no username or key up front. A host is now a **server you add accounts under**: expand it and click **Add User...** to add an account with its own password or private key (with passphrase). This lets one server hold several accounts, each with its own credential. Existing saved hosts keep working unchanged. (Known limitation, being finished next: exporting connections does not yet include brand-new accountless server entries, and FTP's Add User still lists a key option that FTP does not use.)

...
### Extension host model (v1.0.5+)

`package.json` declares `"extensionKind": ["ui"]`. SSH Lite **always** runs on the local **UI extension host** (the user's own machine) — in a regular VS Code window AND inside a Remote-SSH session. SSH connections originate from the user's machine; downloads land on the local filesystem; port forwards bind to the user's machine. When VS Code is connected to a Remote-SSH workspace, the Marketplace shows an **Install in Local** button — the same UX as the PDF Viewer and other UI-only extensions.

**Why a lone `["ui"]`, not `["ui", "workspace"]`?** v0.8.17 through v1.0.4 declared both kinds, intending `"ui"` as the preference and `"workspace"` to allow "chained SSH" (running SSH Lite *from* a remote server to a third machine). But `extensionKind` is a *preference*, not *enforcement*: a copy installed on — or carried over to — the remote workspace host could still be the active instance, and there `getConfiguration('sshLite').get('hosts')` reads the server's empty settings and `os.homedir()` is the server's home. That made the saved host list invisible and Add Host fail inside a Remote-SSH window — the v0.8.17 download bug recurring as a host-list bug (see `.adn/lessons.md` "2026-06-22"). `["ui"]` closes the door: VS Code can never run SSH Lite on the workspace host, so the host list shown is always the user's local one. The trade-off — losing chained SSH — is acceptable because that mode is fundamentally incompatible with "saved hosts always visible". Manifest-level regression net: `src/__tests__/manifest/extensionKind.test.ts`.

The `isOnRemoteWorkspaceHost()` detection, the one-time **Install in Local** activation hint, and the upload point-of-action warning are kept as a **defensive fallback**. With `["ui"]` they effectively never fire (the extension cannot run as `ExtensionKind.Workspace`), but they cost nothing and still help if a future VS Code build ever places the extension on the workspace host against the manifest.
...
**Edge case — port forwarding scope**: since SSH Lite always runs on the UI host, inside a Remote-SSH window the forwarded port lives on the user's machine, not the remote workspace. Tools running inside the Remote-SSH workspace (e.g., `curl` in the Remote-SSH terminal) cannot reach that port. For workspace-side access, use VS Code's built-in Remote-SSH port forwarding.
